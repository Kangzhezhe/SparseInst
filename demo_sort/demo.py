...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            masks = predictions['instances'].pred_masks.numpy().astype(np.uint8)
            q = len(masks)
            dets = []
            for i in range(q):
                bbox = mask2bbox(masks[i])
                score = predictions['instances'].scores[i].numpy()
                det = np.concatenate((bbox, [score]))
                dets.append(det)
            dets = np.array(dets)
            print(dets)
            ...
            # ids = np.argsort(trackers[:, -1])
            ids = trackers[:, -1]

            visualized_output = demo.visualize(
                img, predictions,ids)
